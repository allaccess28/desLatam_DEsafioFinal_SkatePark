...
export const loginForm = (req, res) => {
  res.render("login", { title: "Login Skate Park" });
};

export const registerForm = (req, res) => {
  res.render("register", { title: "Register Skate Park" });
};
...
export const addSkater = async (req, res) => {
  try {
    ...
    await check("password")
      .isLength({ min: 6 })
      .withMessage("Ingresa un password de al menos 6 caracteres")
      .run(req);
    await check("confirm_password")
      .equals(password)
      .withMessage("Las passwords no coinciden")
      .run(req);
    await check("anos_experiencia")
      .notEmpty()
      .withMessage("Ingresa años de experiencia")
      .run(req);
    await check("especialidad")
      .notEmpty()
      .withMessage("Ingresa truco especial")
      .run(req);
  ...
  try {
    ...
    if (email == godEmail && password == godPassword) {
      ...
      res
        ...
        .status(200)
        .redirect("/datos?email=" + encodeURIComponent(userVerify.email));
    }
  } catch (error) {
    res.status(500).send(error.message);
  }
